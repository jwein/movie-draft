...
import { canMemberPickMovie, getCategoryNameById, isMovieEligibleForCategory } from '../utils/rules';
import { getCanonicalMemberIdFromMember } from '../utils/identity';
...
function draftReducer(state, action) {
  switch (action.type) {
    ...
    case ACTIONS.MAKE_PICK: {
      ...
      // Enforce draft rules before mutating state
      const currentMemberObj = state.members.find(m => m.id === currentMemberId) || null;
      const currentMemberCanonicalId = currentMemberObj ? getCanonicalMemberIdFromMember(currentMemberObj) : null;
      ...
      if (!currentMemberCanonicalId) {
        console.error(`Unable to determine canonical member id for member ${currentMemberObj?.name || currentMemberId}`);
        return state;
      }
      const validation = canMemberPickMovie(currentMemberCanonicalId, movie, categoryName, state);
      if (!validation.allowed) {
        console.warn(`[MAKE_PICK blocked] ${validation.reason || 'Invalid pick by rules'}`, {
          member: currentMemberObj?.name,
...
export function useDraftState() {
  ...
  // Filter available movies by category eligibility using centralized rules engine
  const getAvailableMoviesForCategory = useCallback((categoryId, list = availableMovies) => {
    ...
    return list.filter(m => isMovieEligibleForCategory(m, categoryName));
  }, [availableMovies]);
